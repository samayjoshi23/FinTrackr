...
const time_ist_1 = require("./time-ist");
...
exports.scheduledDailyNotifications = (0, scheduler_1.onSchedule)({
    ...
    const now = new Date();
    const dateKey = (0, time_ist_1.istDateKey)(now);
    const { start: istStart, endExclusive: istEnd } = (0, time_ist_1.istDayBoundsUtc)(dateKey);
    const startTs = firestore_1.Timestamp.fromDate(istStart);
    const endTs = firestore_1.Timestamp.fromDate(istEnd);
    const usersSnap = await db.collection('users').get();
    const userIds = usersSnap.docs.map((d) => d.id);
    await Promise.allSettled(userIds.map((uid) => processBudgetsAndGoalsForOwner(db, uid, now, dateKey)));
    ...
    for (const accDoc of accounts) {
        ...
        await processRecurringForAccount(db, accountId, accountName, ownerId, recipients, startTs, endTs, dateKey);
        if ((0, time_ist_1.isLastDayOfMonthIST)(now)) {
            await processMonthEndForAccount(db, accountId, accountName, ownerId, recipients, dateKey);
        }
    }
});
...
async function setMarker(db, userId, markerId) {
    await db.doc(`users/${userId}/cron-markers/${markerId}`).set({ createdAt: firestore_1.FieldValue.serverTimestamp() });
}
async function processRecurringForAccount(db, accountId, accountName, ownerId, recipients, startTs, endTs, dateKey) {
    ...
    for (const rdoc of recurringSnap.docs) {
        ...
        if (rec.isAutoPay) {
            ...
            if (await markerExists(db, ownerId, markerBase))
                continue;
            ...
            const monthKey = (0, time_ist_1.istMonthKey)(new Date(`${day}T12:00:00+05:30`));
            await (0, monthly_report_sync_1.recomputeMonthlyReportForAccount)(accountId, ownerId, monthKey);
            const currency = await getAccountCurrency(db, accountId);
            const amtLabel = formatAmount(amount, currency);
            for (const uid of recipients) {
                await (0, notification_trigger_1.createNotification)(uid, {
            ...
            const next = computeNextPaymentDate(rec.nextPaymentDate.toDate(), rec.recurringFrequency ?? null);
            await rdoc.ref.update({
                lastPaymentDate: firestore_1.Timestamp.fromDate(new Date(`${day}T12:00:00+05:30`)),
                nextPaymentDate: firestore_1.Timestamp.fromDate(next),
            ...
            for (const uid of recipients) {
                ...
                if (await markerExists(db, uid, markerId))
                    continue;
...
async function processMonthEndForAccount(db, accountId, accountName, ownerId, recipients, dateKey) {
    const prevMonth = (0, time_ist_1.istPreviousMonthKey)(new Date(`${dateKey}T12:00:00+05:30`));
    const markerBase = `${dateKey}-MONTH_END-${accountId}`;
    if (await markerExists(db, ownerId, markerBase))
        return;
    await setMarker(db, ownerId, markerBase);
    await (0, monthly_report_sync_1.recomputeMonthlyReportForAccount)(accountId, ownerId, prevMonth);
...
function buildMonthEndCopy(savings, expense, income, prevMonth) {
...
async function processBudgetsAndGoalsForOwner(db, userId, today, dateKey) {
...
async function checkBudgets(db, userId, today, dateKey) {
    const monthKey = (0, time_ist_1.istMonthKey)(today);
    const monthLong = (0, time_ist_1.istMonthLongName)(today);
    ...
    for (const bdoc of budgetsSnap.docs) {
        ...
        if (bm && bm !== monthKey && bm.toLowerCase() !== monthLong.toLowerCase())
            continue;
...
async function sumTransactionsForBudget(db, accountId, category, month) {
...
async function checkGoals(db, userId, dateKey) {
    ...
    for (const gdoc of goalsSnap.docs) {
        ...
        if (!target || goal.currentAmount < target)
            continue;
        if (goal.isCompleted)
            continue;
...
async function getAccountCurrency(db, accountId) {
...
function computeNextPaymentDate(prev, frequency) {
