...
exports.deleteUserAccount = (0, https_1.onCall)({ timeoutSeconds: 300, memory: '512MiB' }, async (request) => {
    if (!request.auth?.uid) {
    ...
    try {
        ...
        for (const accountId of ownedAccountIds) {
            await deleteQueryDocs(db, db.collection('transactions').where('accountId', '==', accountId));
            await deleteQueryDocs(db, db.collection('recurring-transactions').where('accountId', '==', accountId));
            await deleteQueryDocs(db, db.collection('monthlyReports').where('accountId', '==', accountId));
        }
        // ── 3. Delete direct-owned collections ─────────────────────────────
        await deleteQueryDocs(db, db.collection('accounts').where('ownerId', '==', uid));
        await deleteQueryDocs(db, db.collection('budgets').where('ownerId', '==', uid));
        await deleteQueryDocs(db, db.collection('budgetPlans').where('ownerId', '==', uid));
        await deleteQueryDocs(db, db.collection('goals').where('ownerId', '==', uid));
        await deleteQueryDocs(db, db.collection('categories').where('ownerId', '==', uid));
        // ── 4. Delete feedback ─────────────────────────────────────────────
        await deleteQueryDocs(db, db.collection('feedback').where('userId', '==', uid));
        ...
        throw new https_1.HttpsError('internal', 'Account deletion encountered errors. Please contact support if you can still sign in.');
    }
});
async function handleGroupCleanup(db, uid) {
    ...
    for (const d of creatorSnap.docs)
        groupMap.set(d.id, d);
    ...
    for (const [groupId, snap] of groupMap) {
        ...
        const activeMemberIds = Array.isArray(data['activeMemberIds']) ? data['activeMemberIds'] : [];
...
async function anonymizeGroupExpenses(db, groupId, uid) {
    ...
    for (const expenseDoc of expensesSnap.docs) {
        ...
        const paidByIds = Array.isArray(data['paidByIds']) ? data['paidByIds'] : [];
        const paidByNames = Array.isArray(data['paidByNames']) ? data['paidByNames'] : [];
...
async function anonymizeGroupSettlements(db, groupId, uid) {
...
async function removeFromSharedAccounts(db, uid) {
    ...
    for (const accountDoc of sharedSnap.docs) {
        ...
        if (accountDoc.data()['ownerId'] === uid)
            continue;
        const members = Array.isArray(accountDoc.data()['members'])
            ? accountDoc.data()['members']
            : [];
        await accountDoc.ref.update({
            members: members.filter((m) => m['memberId'] !== uid),
            memberIds: firestore_1.FieldValue.arrayRemove(uid),
            activeMemberIds: firestore_1.FieldValue.arrayRemove(uid),
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
        });
    }
}
