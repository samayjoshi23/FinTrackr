...
exports.recomputeMonthlyReportForAccount = recomputeMonthlyReportForAccount;
const firestore_1 = require("firebase-admin/firestore");
const time_ist_1 = require("./time-ist");
function catKey(id) {
    return id.startsWith('cat_') ? id : `cat_${id}`;
}
function monthLongFromKey(monthKey) {
...
function resolveExpenseCategory(categoryName, byLowerName) {
    ...
    if (cat)
        return { id: cat.uid, displayName: cat.name };
    return { id: `unmapped_${stableNameHash(lower)}`, displayName: raw };
}
function filterBudgetsForMonth(monthKey, budgets) {
...
async function recomputeMonthlyReportForAccount(accountId, ownerId, monthKey) {
    ...
    const day = (0, time_ist_1.istDateKey)(new Date());
    const payload = {
        month: monthKey,
        accountId,
        totalIncome,
...
//# sourceMappingURL=monthly-report-sync.js.map